...
- **2026-06-20 (14)** — **Real content + legal pages (per user).** **Mentors:** removed the 4 placeholder mentors; added the two real people supplied by the user — Palash Khurpia (M.Com, LLB, LLM — Taxation Lawyer · Speaker on GST Laws · Litigation Enthusiast) marked **Founder & Owner** and listed first, and Bhupesh Khurpia (M.Com, LLB — Taxation Lawyer). **Mentors color fix:** `SectionHeading` gained a `dark` variant (white/gold text) so the heading is readable on the navy mentors band — was previously dark-on-dark and invisible. **About page** rewritten with honest mission + a Founder bio block for Palash. **Contact page** now names Palash Khurpia as the contact, alongside edvax.info@gmail.com / +91 98268 04435. **Legal pages (all original prose, new routes):** `RefundPolicy` (/refund-policy) — strict, all-sales-final/no-refunds with a narrow erroneous-or-duplicate-charge carve-out, help email; `PrivacyPolicy` (/privacy) — data collected, **Razorpay** as payment processor (we never store card/UPI credentials), DPDP Act 2023 + IT Act framing, user rights, grievance contact; `Terms` (/terms) — courses/access/acceptable-use/IP/liability/governing law (India). Footer now links all three. Shared `LegalPage` layout component. **IMPORTANT for the user:** these are professional drafts, NOT a substitute for legal review — Palash should have a qualified professional confirm they meet Razorpay's onboarding + Indian law before relying on them. **Verified:** frontend builds (~72 kB gzip). **Next:** deploy to VM (these + the pending cookie/auth fix).
